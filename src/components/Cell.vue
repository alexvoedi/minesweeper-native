...
<template>
  <button class="flex items-center justify-center bg-gray border border-gray-300" :class="[css]" :style="{
    width: `${props.cellSize}px`,
    height: `${props.cellSize}px`,
    fontSize: `${0.75 * props.cellSize}px`
  }" @contextmenu.prevent="toggleState()">
    <span v-if="state === State.Flagged" class="ico-mdi-flag text-red-700"></span>
    <span v-else-if="state === State.Question" class="ico-mdi-help"></span>
  </button>
</template>
